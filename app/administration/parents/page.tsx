...
import { KeyRound, Link2, Plus, Search, ShieldCheck, Users, X } from "lucide-react";
...
export default function AdministrationParentsPage() {
  ...
  const handleProvisionAccess = async (parentId: string) => {
    setFormError("");
    setAccessLoading(parentId);
    ...
    try {
      ...
      const response = await fetch("/api/identity/provision", {
        ...
        body: JSON.stringify({ entity: "parent", entity_id: parentId }),
      });
      const result = await response.json().catch(() => null);

      if (!response.ok || !result?.success) {
        setFormError(result?.error || "Impossible de générer l'accès parent.");
        return;
      }
      ...
      if (result.temporaryPassword) {
        ...
        setFormError(result.message || "Ce parent possède déjà un accès.");
      }
    } catch {
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-slate-100 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                {canManage && <th className="px-4 py-3 text-right font-semibold">Accès</th>}
              </tr>
            </thead>
            <tbody>
              {filtered.map((p) => (
                ...
                >
                  ...
                  {canManage && (
                    <td className="px-4 py-3 text-right">
                      <button
                        type="button"
                        onClick={() => handleProvisionAccess(p.id)}
                        ...
                        title="Générer les identifiants du portail parent"
                      >
                        <KeyRound className="h-3.5 w-3.5" />
                        {accessLoading === p.id ? "Génération…" : "Accès"}
                      </button>
                    </td>
                  )}
                </tr>
          ...
          <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-7 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-xl font-bold text-slate-950">Accès parent généré</h2>
              </div>
