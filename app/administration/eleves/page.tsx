...
export default function AdministrationElevesPage() {
  const { schoolId, school, hasPermission, loading: userLoading } =
    useCurrentUser();
  const canRead = hasPermission("students.read") || hasPermission("students.create");
      ...
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            ...
            {school?.name ? ` · ${school.name}` : ""} — une fiche, réutilisée
            partout (PRD).
          </p>
        </div>
        {canCreate && (
          <button
            type="button"
            disabled
            title="Création détaillée à brancher ensuite"
