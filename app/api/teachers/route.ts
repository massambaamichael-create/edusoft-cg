  ...
  try {
    ...
    if (!user || !ALLOWED_CREATOR_ROLES.has(user.role_name ?? "")) {
      return NextResponse.json(
        { success: false, error: "Vous n'avez pas l'autorisation d'accéder à cette ressource." },
        { status: 403 }
      );
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!user || !ALLOWED_CREATOR_ROLES.has(user.role_name ?? "")) {
      return NextResponse.json(
        { success: false, error: "Vous n'avez pas l'autorisation de créer un enseignant." },
        { status: 403 }
      );
    }
    ...
    // The authenticated user's school is authoritative. A client-provided school_id
    // can only be accepted when it matches the authenticated school.
    ...
    // 2. Création du profil dans l'école de l'utilisateur authentifié
    ...
    if (userError || !userData) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: userError?.message ?? "Impossible de créer le profil utilisateur.",
        },
        { status: 500 }
      );
    }
    ...
    // 3. Création du profil enseignant dans la même école
    ...
    if (teacherError || !teacherData) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: teacherError?.message ?? "Impossible de créer le profil enseignant.",
        },
        { status: 500 }
      );
    }
    ...
    if (emailError) {
      ...
      // Avoid leaving an account that cannot receive its credentials.
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Le compte n'a pas pu être finalisé car l'email d'accès n'a pas pu être envoyé.",
        },
        { status: 502 }
      );
    }
    ...
    // Best-effort cleanup for unexpected failures after Auth creation.
    if (createdProfileId) {
      await supabaseAdmin.from("users").delete().eq("id", createdProfileId);
    }
    if (createdAuthUserId) {
    ...
    return NextResponse.json(
      { success: false, error: "Erreur serveur lors de la création de l'enseignant." },
      { status: 500 }
    );
  }
}
