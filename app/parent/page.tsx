...
export default function ParentPortalPage() {
  ...
  const load = async () => {
    ...
    if (!response.ok || !result?.success || result.role !== "Parent") {
      setError(result?.error || "Accès parent indisponible.");
      setLoading(false);
      return;
    }
    ...
    <main className="min-h-screen bg-[#F6F7FB]">
      <header className="border-b border-slate-200/80 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">EduSoft CG</p><h1 className="text-lg font-bold text-slate-950">Espace Parent</h1></div>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-8 lg:px-10">
        <section className="rounded-[28px] bg-slate-900 p-7 text-white shadow-xl">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-bold">Bonjour {data.profile.first_name || "Parent"}.</h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-white/65">Suivez les informations scolaires de vos enfants depuis un seul espace sécurisé.</p>
            </div>
            <div className="flex items-center gap-2 text-xs text-white/65"><ShieldCheck className="h-4 w-4" /> Données limitées à vos enfants</div>
          </div>
        </section>

        <section className="mt-6 grid gap-4 sm:grid-cols-3">
          <Stat icon={<UserRound className="h-4 w-4" />} label="Enfants suivis" value={String(data.students.length)} />
        ...
        <section className="mt-8">
          <div className="mb-4"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Mes enfants</p><h3 className="mt-1 text-xl font-bold text-slate-950">Situation scolaire</h3></div>
