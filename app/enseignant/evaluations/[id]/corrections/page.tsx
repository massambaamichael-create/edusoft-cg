...
type Variant={id:string;variant_code:string;content:any;correction_content:any};
...
function questionsFromContent(content:any):Item[]{
 const qs=Array.isArray(content?.questions)?content.questions:Array.isArray(content)?content:[];
 return qs.map((q:any,i:number)=>({question_key:String(q?.id??q?.key??q?.number??i+1),question_label:String(q?.title??q?.text??q?.question??("Question "+(i+1))),awarded_score:0,max_score:Number(q?.points??q?.max_score??q?.maxPoints??0)||0,student_answer:"",teacher_comment:"",display_order:i}));
}

export default function CorrectionsPage(){
 const {id}=useParams<{id:string}>();const router=useRouter();
 const [assessment,setAssessment]=useState<any>(null),[students,setStudents]=useState<Student[]>([]),[assignments,setAssignments]=useState<Assignment[]>([]),[variants,setVariants]=useState<Variant[]>([]),[sessions,setSessions]=useState<Session[]>([]),[items,setItems]=useState<Item[]>([]),[selected,setSelected]=useState<string|null>(null),[feedback,setFeedback]=useState(""),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState(""),[message,setMessage]=useState("");
 ...
 async function load(){
  ...
  const ids=(aa.data??[]).map((x:any)=>x.student_id);let st:any[]=[];
  if(ids.length){const r=await supabase.from("students").select("id,first_name,last_name,registration_number").in("id",ids).order("last_name");if(r.error){setError(r.error.message);setLoading(false);return}st=r.data??[]}
  ...
  setAssessment(ar.data);setVariants(vr.data??[]);setAssignments(aa.data??[]);setStudents(st);setSessions(sr.data??[]);setSelected(st[0]?.id??null);setLoading(false);
 }
 ...
 function updateItem(i:number,key:keyof Item,value:any){setItems(x=>x.map((it,n)=>n===i?{...it,[key]:value}:it))}
 ...
 const label=(s:string)=>({draft:"Brouillon",submitted:"Soumise",validated:"Validée",rejected:"À reprendre"} as any)[s]??s;
 ...
 return <main className="min-h-screen bg-[#F7F8FC] p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
  ...
  <header><p className="text-sm font-semibold text-violet-700">Correction & notes</p><h1 className="mt-1 text-2xl font-bold text-slate-950">{assessment.title}</h1><p className="mt-2 text-sm text-slate-500">Correction question par question · note normalisée sur 20 · validation humaine avant inscription officielle de la note.</p></header>
  ...
  <div className="grid gap-5 lg:grid-cols-[300px_1fr]">
   <aside className="rounded-2xl border bg-white p-4 shadow-sm"><h2 className="font-bold text-slate-900">Élèves ({students.length})</h2><div className="mt-3 space-y-1">{students.map(s=>{const se=sessions.find(x=>x.student_id===s.id);return <button key={s.id} onClick={()=>setSelected(s.id)} className={"w-full rounded-xl p-3 text-left "+(selected===s.id?"bg-violet-50 text-violet-900":"hover:bg-slate-50")}><div className="flex items-center justify-between"><span className="font-semibold">{s.last_name} {s.first_name}</span>{se?.status==="validated"&&<CheckCircle2 className="h-4 w-4 text-emerald-600"/>}</div><p className="mt-1 text-xs text-slate-400">{s.registration_number??"Matricule —"} · {se?label(se.status):"Non corrigé"}</p></button>})}</div></aside>
   <section className="rounded-2xl border bg-white p-5 shadow-sm">{!currentStudent?<p className="text-sm text-slate-500">Aucun élève à corriger. Répartissez d’abord les variantes depuis l’évaluation.</p>:<>
    <div className="flex flex-col gap-3 border-b pb-5 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-bold">{currentStudent.last_name} {currentStudent.first_name}</h2><p className="text-sm text-slate-500">Variante {currentVariant?.variant_code??"—"} · {currentSession?label(currentSession.status):"Brouillon"}</p></div><div className="text-right"><p className="text-xs text-slate-400">Note</p><p className="text-2xl font-bold text-violet-700">{currentSession?.normalized_score??"—"} / 20</p></div></div>
    <div className="mt-5 space-y-4">{items.length?items.map((it,i)=><div key={it.question_key} className="rounded-xl border border-slate-200 p-4"><div className="flex justify-between gap-4"><p className="font-semibold text-slate-900">{it.question_label}</p><span className="text-xs text-slate-400">/{it.max_score}</span></div><textarea value={it.student_answer} onChange={e=>updateItem(i,"student_answer",e.target.value)} placeholder="Réponse / observation de la copie" className="mt-3 min-h-20 w-full rounded-lg border p-3 text-sm"/><div className="mt-3 grid gap-3 md:grid-cols-[140px_1fr]"><input type="number" min="0" max={it.max_score} step="0.25" value={it.awarded_score} onChange={e=>updateItem(i,"awarded_score",e.target.value)} className="rounded-lg border p-2.5 text-sm"/><input value={it.teacher_comment} onChange={e=>updateItem(i,"teacher_comment",e.target.value)} placeholder="Commentaire du correcteur" className="rounded-lg border p-2.5 text-sm"/></div></div>):<div className="rounded-xl bg-slate-50 p-5 text-sm text-slate-500">Le sujet ne contient pas de questions structurées. Ajoutez des questions structurées dans le sujet IA pour activer la correction détaillée.</div>}</div>
    <textarea value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Appréciation générale de la copie" className="mt-5 min-h-24 w-full rounded-xl border p-3 text-sm"/>
    <div className="mt-5 flex flex-wrap gap-2"><button disabled={saving||currentSession?.status==="validated"} onClick={()=>void save(false)} className="inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold"><Save className="h-4 w-4"/> Enregistrer</button><button disabled={saving||currentSession?.status==="validated"} onClick={()=>void save(true)} className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{saving?<Loader2 className="h-4 w-4 animate-spin"/>:<Send className="h-4 w-4"/>} Soumettre la correction</button></div>
   </>}</section>
  </div>
 </div></main>;
}
